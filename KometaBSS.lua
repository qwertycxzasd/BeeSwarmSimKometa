...
if syn then
    pcall(function()
        temptable.WebSocket = syn.websocket.connect("ws://api.kometa.pw:8888/")
    end)
elseif Krnl then
    pcall(function()
...
-- else 
--     api.notify('kometa', 'Your executor is not supported and you will not be able to use some features. (they will not be displayed)')    
--     task.wait(9999999)
end
...
--                         if v:FindFirstChild("TimerLabel", true).Visible == false then
--                             cocopad.CanCollide = true
--                         else 
--                             cocopad.CanCollide = false
--                             for i = 1, 10 do gettoken(v.Territory.Value.Position) end
--                         end
--                     end
--                 end
--             end
        ...
        if v.Name ~= "Ant Challenge Info" and v.Name ~= "Bubble Bee Man 2" and v.Name ~= "Wind Shrine" and v.Name ~= "Gummy Bear" then if v:FindFirstChild("Platform") then if v.Platform:FindFirstChild("AlertPos") then if v.Platform.AlertPos:FindFirstChild("AlertGui") then if v.Platform.AlertPos.AlertGui:FindFirstChild("ImageLabel") then
            ...
            if image.ImageTransparency == 0 then
                if kometa.toggles.tptonpc then
                    ...
                    api.tween(5,CFrame.new(v.Platform.Position.X, v.Platform.Position.Y+3, v.Platform.Position.Z))
                    task.wait(3)
                end
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(v.Platform.Position.X, v.Platform.Position.Y+3, v.Platform.Position.Z)
                task.wait(.1)
...
game.Players.PlayerRemoving:Connect(function(player)
    ...
    if kometa.webhooking.connectionlost then api.imagehook(kometawebhook.webhook, "You've been disconnected!\nTotal Honey: `"..game:GetService("Players").LocalPlayer.PlayerGui.ScreenGui.MeterHUD.HoneyMeter.Bar.TextLabel.Text.."`\nGained Honey: `"..api.suffixstring(temptable.honeycurrent - temptable.honeystart).."`\nElapsed Time: `"..api.toHMS(temptable.stats.runningfor).."`", "kometa â˜„ï¸", "https://static.wikia.nocookie.net/bee-swarm-simulator/images/7/7f/Eviction.png/revision/latest?cb=20200403235248", kometa.webhooking.ping, "<@"..kometa.webhooking.discordid..">") end
end)

game.Players.PlayerAdded:Connect(function(player)
    if kometa.webhooking.playeradded then api.imagehook(kometawebhook.webhook, player.Name.." joined your server", "kometa â˜„ï¸", "https://www.roblox.com/headshot-thumbnail/image?userId="..player.UserId.."&width=420&height=420&format=png", kometa.webhooking.ping, "<@"..kometa.webhooking.discordid..">") end
end)
